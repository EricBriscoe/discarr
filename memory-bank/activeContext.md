...
- **Async calls**: Await every coroutine; an omitted `await` caused the recent bug.
...
- **Testing Strategy**: The bug was not caught by existing tests, indicating a need for integration or command-level tests.
...
- Command-level tests should cover the failed paths described above.
