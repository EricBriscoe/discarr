...
export default function Blocked() {
  ...
  async function refresh() {
    ...
    try { const data = await getBlocked(); setRadarr(data.radarr || []); setSonarr(data.sonarr || []); }
  ...
  async function handleReject(service:'radarr'|'sonarr', id:number) {
    try { await rejectBlocked(service, id); await refresh(); } catch (e:any) { window.alert(e.message); }
  }

  const Section = ({name, items, service}:{name:string; items:Block[]; service:'radarr'|'sonarr'}) => (
          ...
          <tbody>
            {items.map(i => (
              <tr key={`${service}-${i.id}`}>
                ...
                <td>
                  <div className="row">
                    <button className="primary" onClick={()=>handleApprove(service,i.id)}>Approve</button>
